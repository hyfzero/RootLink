...
```
data/
├── amadues/                    # Brain ID: amadues
│   ├── persona/
│   │   ├── profile.json
│   │   └── memories.json       # 包含 daily_summary_memories, monthly_summary_memories
│   ├── history/
...
│   ├── history/
│   └── tags/
├── brain_001/           # brain_001（独立）
│   ├── persona/
│   ├── history/
│   └── tags/
└── brain_002/          # brain_002（独立）
    ├── persona/
    ├── history/
    └── tags/
```
...
ReplyTagger.generate_tag()
    ↓
保存到 data/{brain_id}/tags/reply_tags.json
    ↓
...
### 3.5 日终/月终摘要生成 (`summarizer.py`)

**目标**: 日期切换时调用 LLM 生成当日对话摘要，月份切换时生成月度总结。
...
- 日终/月终摘要同时更新 persona 记忆文件 (`daily_summary_memories` / `monthly_summary_memories`)
...
**日终摘要内容结构** (JSON + Markdown):
```json
{
    "date": "2026-03-28",
    "summary_text": "2-3句话的对话摘要",
    "important_messages": ["最重要的1-2条消息"],
    "topics": ["讨论的主要话题"],
...
{
    "year_month": "2026-03",
    "summary_text": "2-3句话的月度总结",
    "major_events": ["本月最重要的1-3个事件"],
    "monthly_topics": ["本月讨论的主要话题"],
    "overall_emotional_tone": "本月整体情感基调",
    "user_long_term_preferences": ["用户长期偏好"],
    "unfinished_monthly_topics": ["本月未完成话题"],
    "growth_or_change": ["用户成长或变化"]
}
```
...
日期检测: today != _current_date
         ...
         ▼
┌──────────────────┐
│ 归档当日 Session │ → data/{brain_id}/history/daily/YYYY-MM-DD.json
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
...
└────────┬─────────┘
         ...
         ▼
┌──────────────────┐
│ 保存到 Brain     │ → data/{brain_id}/history/summaries/YYYY-MM-DD.summary.md/.json
│ 更新记忆         │ → data/{brain_id}/persona/memories.json (daily_summary_memories)
└──────────────────┘
```

**月份切换（月终摘要）**:
```
月份检测: current_month != today_month
         │
         ▼
┌──────────────────┐
│ 收集当月每日摘要  │
...
└────────┬─────────┘
         ...
         ▼
┌──────────────────┐
│ 保存到 Brain     │ → data/{brain_id}/history/summaries/YYYY-MM.monthly.md/.json
│ 更新记忆         │ → data/{brain_id}/persona/memories.json (monthly_summary_memories)
└──────────────────┘
```
...
| **写入 persona/memories.json** | ✅ | ✅ (更新情景/偏好/事实/日终/月终记忆) |
