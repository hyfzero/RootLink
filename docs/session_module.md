...
TagGenerator 支持两种情感解析模式，SessionManager 暴露以下接口供 UI 层调用：

```python
def get_emotion_mode(self) -> str
```
获取当前情感解析模式。返回 `"keyword"` 或 `"llm"`。
...
| mode | 说明 |
|------|------|
| `"keyword"` | 使用多语言关键词匹配，速度快，无需 LLM 调用 |
| `"llm"` | 使用 LLM 解析，精度更高，但需要 LLM 调用 |

```python
def is_llm_emotion_enabled(self) -> bool
```
检查是否启用 LLM 情感解析。返回 `True` 表示当前为 `"llm"` 模式。

**使用示例**：
```python
# 查看当前模式
current = session_manager.get_emotion_mode()  # "keyword" 或 "llm"

# 切换到 LLM 模式
session_manager.set_emotion_mode("llm")

# 检查是否 LLM 模式
if session_manager.is_llm_emotion_enabled():
    print("使用 LLM 解析情感")
...
**注意**：`emotion_mode` 存储在 `TagGenerator` 实例上，在 `SessionManager` 初始化时若传入了自定义 `TagGenerator`（如 `session_example.py` 中的 `TagGenerator()`），系统会自动补充 LLM 调用函数，无需手动设置。
