...
符号采用小型拉丁字体和线段绘制，不加载整套中文字库。界面仅在 `voice` 命令中打开。LVGL 在主线程运行，语音在工作线程运行，状态通过线程安全快照更新。关闭窗口或 Ctrl+C 取消处理，等待释放音频与 Python 后再退出。发生终止性故障时窗口保留 `×`；点击屏幕可取消旧运行，待资源释放后创建新运行并恢复已保存的数据，不重放消息。正常处理期间也可点击重置。ASR/TTS 非致命超时自动跳过该轮并恢复收音。板端触摸需要另设 `UI_INPUT_DEVICE`，详见用户手册。`UI_BACKEND=none` 保持原有命令行行为。
