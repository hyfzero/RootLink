...
窗口上半部分显示语音状态，下半部分逐字显示人格返回的原文中文回答。文字满三排后默认停留 2 秒，清屏后继续逐字显示下一页；最后一页保留，不再清屏。`UI_PAGE_HOLD_MS=2000` 控制满页停留时长，允许 0–10000 ms，并可由同名环境变量覆盖。`UI_TEXT_INTERVAL_MS=50` 仍独立控制每个 Unicode 字符的显示间隔，可在 10–1000 ms 调整，并可由同名环境变量覆盖。点击窗口重置会清空当前回答显示；日语翻译仍只供 TTS 使用，显示不会切换为日语，也不保证与播放逐词同步。
