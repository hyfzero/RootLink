...
启用界面时，下半屏按日语播报的句段显示对应中文：首帧音频写入声卡成功后才开始逐字显示，速度根据该段 PCM 时长自动计算，每段从新页开始。每页最多三行，满页按 `UI_PAGE_HOLD_MS` 停留（默认 2000 ms，范围 0–10000 ms），再清屏继续；本段音频和字幕都完成后才能进入下一段，最后一页保留。`UI_TEXT_INTERVAL_MS` 只作为非同步打字组件的备用间隔，不控制当前语音字幕速度。点击重置会取消音频、预取和未显示字幕，不重播。日语译文不替换中文，也不写入人格历史。这是句段级同步，未使用跨语言逐字时间戳，声卡缓冲仍会带来少量延迟。

`VoiceRuntime` 在最终回答上分句，长句按标点优先拆成有界句段。仅启用语音字幕观察器时使用分段管线；无界面保留整段合成。每段独立经过现有翻译和 TTS，当前段播放期间用单个 future 预取下一段，TTS 调用串行，最多保留当前与下一段音频。工作线程发布中文、音频时长和播放起点；主线程操作 LVGL 并按段版本确认展示完成。播放 drain 后可取消地等待字幕确认，重置不会让旧确认结束新段。等待上限复用 `PERSONA_TURN_TIMEOUT_MS`（默认 300 秒），超时结束本轮并回收预取，避免字幕确认丢失时无限等待。分页停留冻结打字时钟，超页句段的总展示时间可能超过音频时长，此时后续音频等待字幕完成。失败时取消并回收预取，再结束本轮；不自动重试或重放人格调用。

2026-09-08 增加 `TTS_TRANSLATE_TO=ja`：C++ 在合成阶段独立调用配置中的 LLM 做无状态翻译，输入只有翻译指令和本次待播文本（界面模式为当前句段），不加载人格 Prompt、工具或历史。它不属于人格对话调用，不触发 `SessionManager` 的状态更新或摘要。缺省 `none` 时直接合成原文。翻译超时由 `TTS_TRANSLATION_TIMEOUT_MS` 控制；失败不退回原文播报，不重放人格轮次。配置与音色管理见 [JAPANESE_VOICE.md](JAPANESE_VOICE.md)。
...
图展示主路径；任一设备或服务阶段都可能失败或取消。ASR/TTS 非致命错误跳过当前轮并恢复采集，不自动重放，也不发布锁存 Error。Python Thinking 阶段失败意味着子进程不可复用，停止当前运行；配置、鉴权、存储、声卡错误同样停止。终止日志保留 Error，清理不覆盖为空闲。界面故障保持到用户点击重置或退出。
...
MainView 将 SDL 左键/触摸松开、板端 `UI_INPUT_DEVICE` 的 evdev `BTN_TOUCH` 松开合并为重置请求。板端以非阻塞方式限量读取输入，不需要坐标标定。主线程先设置取消标志，继续刷新界面，直到工作线程完成；随后 join、清除状态锁存与取消标志、启动全新工作线程。旧 provider、声卡及 Python 进程销毁后才创建新实例，不并发运行两个人格核心。重新启动读取相同数据目录和运行配置，不复制或重放消息。无界面模式的退出方式不变。

取消响应仍受底层调用边界约束：HTTP 有请求超时与取消检查，Python IPC 轮询检查取消，ALSA 写入和 drain 有 2 秒上限。主线程不在重置请求时阻塞 join；这些措施不等于对内核驱动挂死的硬件看门狗保障。
